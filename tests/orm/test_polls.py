...
from poll.orm.helpers.polls.get_poll_lines import get_poll_lines
from poll.orm.helpers.polls.rebuild_buttons import rebuild_buttons
from poll.orm.helpers.polls.votes import add_vote, remove_vote, reset_votes
...
class TestPolls(IsolatedAsyncioTestCase, unittest.TestCase, BotTest):
    ...
    async def test_poll_votes(self):
        ...
        await poll.insert()
        await poll.set_default_games(self.guild)

        poll = await rebuild_buttons(poll)
